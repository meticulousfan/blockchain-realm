...
contract NFTsRealmMarketplace is ReentrancyGuard, Ownable {
    ...
    function buyNew(uint256 _id, 
        ...
        //    NFTsRealm(nftContract).transferFrom(msg.sender, payoutAddressMap[paymentType], _commissionValue.div(2));
            ...
            transferKCSOrWKCS(payable(adminRecoveryAddress),_commissionValue.mul(6).div(10));
            transferKCSOrWKCS(payable(payoutAddressMap[paymentType]),_commissionValue.mul(4).div(10));
            ...
            require(IERC20(tokenAddressMap[paymentType]).transferFrom(msg.sender, adminRecoveryAddress, _commissionValue.mul(6).div(10)));
    ...
    function buy(uint256 _id, uint256 _price, string memory paymentType) external payable {
        ...
        if (keccak256(abi.encodePacked((paymentType))) == keccak256(abi.encodePacked(("BNB")))) {
            ...
            transferKCSOrWKCS(payable(adminRecoveryAddress), _commissionValue.mul(4).div(10));
            transferKCSOrWKCS(payable(payoutAddressMap[paymentType]), _commissionValue.mul(4).div(10));
        } else {
            ...
            require(IERC20(tokenAddressMap[paymentType]).transferFrom(msg.sender, adminRecoveryAddress, _commissionValue.mul(4).div(10)));
            require(IERC20(tokenAddressMap[paymentType]).transferFrom(msg.sender, payoutAddressMap[paymentType], _commissionValue.mul(4).div(10)));
        }
        NFTsRealm(nftContract).transferFrom(address(this), msg.sender, _id);
        ...
        onlyCreatorOrWinner(tokenId)
    {
        ...
        if(winner == address(0)) {
            ...
            if (keccak256(abi.encodePacked((paymentType))) == keccak256(abi.encodePacked(("BNB")))) {
                ...
                require(IERC20(tokenAddressMap[paymentType]).transfer(adminRecoveryAddress, _commissionValue.mul(4).div(10)));
